package net.dkahn.starter.apps.webapps.frontend.authentication;
...
public class AuthenticationUserService extends GenericRepositoryJpa<User, Long> implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if(user==null){
            ...
            if(!user.getProfiles().contains(profileRepository.findByName(IProfileRepository.FRONT_PROFILE))){
                throw new UsernameNotFoundException("user '" + username + "' not found...");
            }
        }

        return new BOUser(user,profileRepository.loadGrantedAuthorities(user));
    }
